...
export const verifyAddHewan = (request: Request, response: Response, next: NextFunction) => {
    ...
    const { error } = addHewanSchema.validate(request.body, {abortEarly: false})
...
export const verifyEditHewan = (request: Request, response: Response, next: NextFunction) => {
    // validasi data dari request body dan mengambil info error jika terdapat error
    const { error } = updateHewanSchema.validate(request.body, { abortEarly: false })
